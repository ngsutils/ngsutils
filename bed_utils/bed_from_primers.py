...
import os,sys,urllib2,re,urllib,time
...
def insilico_pcr_fasta(tab,db='hg18',perfect=15,good=15,size=4000,flip=False):
    primers = {}
    names = []
    with open(tab) as f:
        seq = ''
...
def insilico_pcr_pairs(fwd,rev,name='primers',db='hg18',perfect=15,good=15,size=4000,flip=False):
        params = [('db',db),('wp_perfect',perfect),('wp_good',good),('boolshad.wp_flipReverse','0'),
                  ('wp_f',fwd),('wp_r',rev),('Submit','submit'),('wp_size',size)]
        if flip:
